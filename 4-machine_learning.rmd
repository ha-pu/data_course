...
```{r}
model_nhst <- glm(
  repurchase ~ customer_tier.2 + customer_tier.3 + customer_sex.male,
  data = train_df,
  family = binomial
)
```
...
comparison_alg %>%
  ...
  geom_hline(
    aes(yintercept = max(Accuracy_base)),
...
comparison_all %>%
  ...
  geom_hline(
    aes(yintercept = max(comparison_all$Accuracy_base)),
    colour = "darkred", linetype = "dashed"
  ) +
  geom_hline(
    aes(yintercept = max(comparison_all$OOS_Acc)),
