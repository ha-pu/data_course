...
accuracy_nhst <- mean(train_df$repurchase == as.integer(pred_train))
```
...
comparison_alg <- data.frame(
  Model = c("glm", "rf", "glmnet", "nnet", "xgbTree", "xgbLinear"),
  Accuracy_base = c(
    accuracy_nhst, max(model_rf$results$Accuracy),
...
OOS_ACC <- bind_rows(
  tibble(
    Model = c("Stacked Tree", "Stacked GLM", "Stacked GBM"),
    OOS_ACC = map_dbl(list(stack_tree, stack_glm, stack_gbm), det_OOS_accuracy)
  ),
  tibble(
    Model = c("rf", "glmnet", "nnet", "xgbTree", "xgbLinear"),
    OOS_ACC = map_dbl(models, det_OOS_accuracy)
  )
)
