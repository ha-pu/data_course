...
```{r}
ensemble_tree <- predict(stack_tree, test_df) %>%
  as_tibble() %>%
  select(ensemble_tree = X1)
ensemble_glm <- predict(stack_glm, test_df) %>%
  as_tibble() %>%
  select(ensemble_glm = X1)
ensemble_gbm <- predict(stack_gbm, test_df) %>%
  as_tibble() %>%
  select(ensemble_gbm = X1)

Results <- as_tibble(predict(models, test_df)) %>% 
  bind_cols(ensemble_tree, ensemble_glm, ensemble_gbm)
